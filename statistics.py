...
if __name__ == "__main__":
    ...
    if args.rebuild_cache:
        df = pd.concat(map(cache(args.cache_path, duration=timedelta(0))(get_dataframe_by_path), args.path))
    else:
        df = pd.concat(map(cache(args.cache_path)(get_dataframe_by_path), args.path))
